...
class DecisionEngine:
    ...
    def _is_safe(self, plan: List[Tuple[str, float]]) -> bool:
        is_safe, _ = self.forecaster.simulate(self.req_date, days=90, candidate_plan=plan)
        return is_safe

    def get_candidate_plans(self) -> List[Dict[str, Any]]:
        candidates = []
    ...
    def decide(self) -> Dict[str, Any]:
        candidates = self.get_candidate_plans()
        best = self.rank_candidates(candidates)
        ...
        if best:
            ...
            if method == 'full_payment':
                ...
                plan_str = "|".join([f"{d}:{a}" for d, a in best['plan']])
            ...
            # Maybe affordable_later if wait was not eligible but full_payment will be safe?
